...
percent_1990_jailpop_wa <- (sum_1990_jailpop_incarceration_wa/sum(wa_1990_incarceration_trends$total_pop)) *100
sum_2018_jailpop_incarceration_wa <- sum(wa_2018_incarceration_trends$total_jail_pop)
percent_2018_jailpop_wa <- (sum_2018_jailpop_incarceration_wa/sum(wa_2018_incarceration_trends$total_pop)) *100
percent_increase_incarceration_wa <- percent_2018_jailpop_wa - percent_1990_jailpop_wa
...
# percentage of black people in jail in each county

wa_incarceration_trends <-mutate(wa_incarceration_trends,
                                 latinx_jail_pop_by_totalpop = (latinx_jail_pop/total_jail_pop)*100)

#percentage of latino people in jail in each county

wa_incarceration_trends <-mutate(wa_incarceration_trends,
                                 latinx_pop_percentage = (latinx_pop_15to64/total_pop)*100)

#percentage of latino people overall in each county
...
#percentage of black population in jail grouped by year in Washington state 
...
scatterplot <- ggplot(data = wa_1990_incarceration_trends) +
  ...
  labs(
    title = "Percentage of Latino population in jail vs Percentage of Latino population overall", # plot title
    x = "Percentage of Latino population overall", # x-axis label
